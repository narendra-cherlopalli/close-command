...
with tabs[0]:
    ...
    c4.metric("FX Diff (Cross-CCY)",   str(fxdiff_count), delta="FX translation gap", delta_color="off")
    c5.metric("Exceptions",            str(exception_count), delta_color="inverse")
    ...
    def _cs(v): return {"Matched":"background-color:#e8f5e9;color:#1A7A4A","Tolerance":"background-color:#fff8e1;color:#A86800","Exception":"background-color:#ffebee;color:#C62828","FX_Diff":"background-color:#E8F4FD;color:#1565C0"}.get(v,"")
...
with tabs[1]:
    ...
    m2.metric("FX Diff (Cross-CCY)", len(fxdiffs), delta="Translation gap — expected", delta_color="off")
    m3.metric("True Exceptions",   len(exceptions), delta_color="inverse")
    m4.metric("Total Gap USD",     f"${mdf['gap_usd'].sum():,.0f}")

    st.divider()
    tm, tfx, te = st.tabs(["✅ Matched", "💱 FX Diff (Cross-Currency)", "⚠️ Exceptions"])
