...
Helios Chemicals Group — Agentic Financial Close Engine.
"""
...
def _render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown("**Helios Chemicals Group**")
        st.divider()
        ...
        run_disabled = st.session_state.get("pipeline_running", False)
        if st.button("Run Close Pipeline", disabled=run_disabled, type="primary"):
            _run_pipeline(selected_period, selected_scenario, uploaded_file)
            st.rerun()
...
def _render_welcome() -> None:
    ...
    st.markdown(
        ...
        <h3 style='text-align:center;color:#888'>Agentic Financial Close Engine — Helios Chemicals Group</h3>
        """,
