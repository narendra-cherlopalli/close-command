...
FX_RATES = {
    "USD": 1.000, "EUR": 1.080, "GBP": 1.270,
    "XAF": 0.001621,   # CFA Franc BEAC (Austral Nitro region)
    "XOF": 0.001621,   # CFA Franc BCEAO (Pinnacle Engineering region)
    "SAR": 0.267,      # Saudi Riyal (Helios Green / Gulf entities)
    "CNY": 0.138,      # Chinese Yuan
    "SGD": 0.741,      # Singapore Dollar
...
with tabs[0]:
    ...
    def _cs(v): return {"Matched":"background-color:#e8f5e9;color:#1A7A4A","Tolerance":"background-color:#fff8e1;color:#A86800","Exception":"background-color:#ffebee;color:#C62828"}.get(v,"")
...
with tabs[1]:
    ...
    m4.metric("Cross-CCY Pairs", len(df[df["seller_ccy"]!=df["buyer_ccy"]]))
        ...
        if len(exceptions)>0:
            for _,row in exceptions.iterrows():
                ...
                with st.expander(f"⚠️ {row['id']} — {s_name} ↔ {b_name} — Gap: {row['gap_pct']:.2f}%"):
                    ...
                    is_xccy = row["seller_ccy"] != row["buyer_ccy"]
                    cause = (f"Cross-currency pair ({row['seller_ccy']} / {row['buyer_ccy']}) — "
                             f"gap likely driven by FX translation at closing rate. "
                             f"Verify XAF/XOF closing rate applied consistently on both sides."
                             if is_xccy else
                             f"Same currency pair — gap of {row['gap_pct']:.2f}% may indicate "
                             f"period cut-off mismatch, accrual vs cash basis, or unapproved adjustment.")
                    st.info(f"**AI Root Cause:** {cause}")
        else:
...
with tabs[2]:
    ...
    edf["Amount"] = edf["Amount"].map("${:,.0f}".format)

    def _drcr(row):
        return ["background-color:#fff4f4"]*len(row) if row["Dr/Cr"]=="Dr" else ["background-color:#f4fff8"]*len(row)

    st.dataframe(edf.style.apply(_drcr,axis=1), use_container_width=True)
...
with tabs[3]:
    ...
    st.caption(f"Complete postings for {period_label} · Helios Chemicals Group · ACTUAL")
    ...
    st.download_button("⬇️ Download Journal Entries (Excel)", buf_je.getvalue(),
                       file_name=f"Helios_IC_Eliminations_{period_label.replace(' ','_')}.xlsx",
    ...
    for _,row in filtered.iterrows():
        ...
        for e in compute_elimination(row):
            audit.append({
                ...
                "Rule":        row["rule_code"],
                "Flow":        row.get("flow",""),
                "Acct Code":   row.get("account_code",""),
                "Account":     e["Account"],
                "Dr/Cr":       e["Dr/Cr"],
...
|---|---|---|---|---|---|
| 1 | Helios IC Source Upload (xlsx/csv) | Bilateral matching engine | Matched / Exception / Tolerance | System | Auto |
| 2 | Account code + UD3 rule attribute | Rule determination (HFM logic) | Elimination rule code (IC-001, INV-001 etc.) | System | Auto |
...
| 5 | Approved journals | Ledger posting | Posted [Elimination] entries in HELIOS application | Finance System | Post HITL |
| 6 | Posted entries | Consolidation proof | Helios Group financial statements | CFO / External Auditor | Final |
""")
    st.info(
        "👤 **Step 4 is a hard gate.** No elimination posts without a named, timestamped, "
        "SQLite-persisted approval. Decisions survive browser refresh and session restart. "
        "Minority interest entries (Austral Nitro 49% NCI, Regal Chemicals 58% NCI) "
        "require additional scrutiny — NCI split is computed at runtime from PCon."
    )
